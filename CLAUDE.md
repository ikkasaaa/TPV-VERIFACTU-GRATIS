...
## Reparto actual: las dos cuentas están en abacosoftware.com

El reparto por sitio de la regla 2 no sirve ahora mismo. El cliente ha pedido que
las **dos cuentas trabajen a la vez sobre abacosoftware.com**, así que
`abacosoftware-seo/` deja de tener un solo dueño y se parte en dos carriles que
no se pisan.

| Carril | Carpeta | Trabajo |
|---|---|---|
| **CONTENIDO** | `abacosoftware-seo/contenido/` | Escribir y profundizar páginas. El pendiente grande: las ~30 páginas de sector más flojas. |
| **TÉCNICO** | `abacosoftware-seo/pipeline/` | Capa técnica, metadatos, títulos, enlazado y sitemap, y retirar los 11 ficheros de `HALLAZGOS_RAIZ_WEB.md`. |

Fuera de esos dos carriles no escribe nadie sin avisar **antes** en su fichero de
estado: `motor/`, `abacosoftware-seo/plantilla.py`, el `driver.py` del skill,
este `CLAUDE.md`, `inventarios/`, `informes/` y todo `carrito5-seo/`.

### Cómo se identifica cada cuenta

No hay chat entre las dos sesiones, así que la identidad **es la rama**, y quien
la declara es `estado/<rama, con / cambiado por ->.md`. Antes de tocar nada:
`git fetch origin main` y leer `estado/` entero, no solo el propio.

### Quién coge qué carril, sin negociar

Un carril es tuyo cuando lo has declarado en tu `estado/` **y empujado**. Si al
rebasar te encuentras con que la otra cuenta declaró el mismo carril, no se
discute ni se le pregunta al cliente: **se queda el carril la rama cuyo nombre
va antes alfabéticamente**, y la otra se pasa al carril libre y lo anota. La
regla es determinista a propósito: las dos llegan solas a la misma conclusión sin
poder hablarse.
