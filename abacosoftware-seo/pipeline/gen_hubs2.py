#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Hubs de los clusters nuevos: abrir un negocio, operativa diaria y normativa.
...
def titulo_de(f):
    """(H1, subtitulo) de una pagina ya generada, para su tarjeta en el hub."""
    s = M.leer(os.path.join(OUT, f))
    m = re.search(r'<p class="hero-subtitulo">(.*?)</p>', s, re.S)
    return M.h1(s) or f, (M.plano(m.group(1)) if m else "")[:150]


def hub(fichero, title, desc, kw, h1, sub, badge, intro, secciones, faqs, links):
    cuerpo = Q.intro(intro) + "".join(Q.seccion_tarjetas(n, [("/" + f, t, d) for f, t, d in items])
                                      for n, items in secciones)
...
def existentes(ficheros):
    return [(f, *titulo_de(f)) for f in ficheros if os.path.exists(os.path.join(OUT, f))]


def main():
    # --------------------------------------------------------------- abrir
    abrir = sorted(os.path.basename(f) for f in glob.glob(os.path.join(OUT, "abrir-*.asp")))
    items_abrir = existentes([f for f in abrir if f != "abrir-un-negocio.asp"])
